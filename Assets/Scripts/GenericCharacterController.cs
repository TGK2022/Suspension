...
public class GenericCharacterController : MonoBehaviour
{
    ...
    public System.IObservable<Vector3> Moved => _moved;
    private Subject<Vector3> _moved;

    public ReactiveProperty<bool> IsRunning => _isRunning;
    private ReactiveProperty<bool> _isRunning;

    public System.IObservable<Unit> Landed => _landed;
    private Subject<Unit> _landed;

    public System.IObservable<Unit> Jumped => _jumped;
    private Subject<Unit> _jumped;

    public System.IObservable<Unit> Stepped => _stepped;
    ...
    private void HandleLocomotion()
    {
        ...
        var jumpLatch = LatchObservables.Latch(this.UpdateAsObservable(), characterInputController.Jump, false);

        // Handle move:
        characterInputController.Move
            ...
            .Subscribe(i =>
            {
                if (!enabled)
                    ...
                    // Horizontal movement:
                    var currentSpeed = characterInputController.Run.Value ? runSpeed : walkSpeed;
    ...
    private void HandleLocomotionCharacterSignalsIteration(bool wasGrounded, bool isGrounded)
    {
        ...
        if (wasGrounded && isGrounded)
        {
            ...
            if (_characterController.velocity.magnitude > 0)
            {
                ...
                tempIsRunning = characterInputController.Run.Value;
            }
        }
